...
export default function LoginPage() {
  ...
  if (status === "loading") return (
    <div className="min-h-screen flex items-center justify-center" style={{ background: "linear-gradient(135deg, #071E3D 0%, #123C69 50%, #2563EB 100%)" }}>
      <div className="flex flex-col items-center gap-3">
        <div className="w-10 h-10 border-3 border-white/30 border-t-blue-500 rounded-full animate-spin" />
  ...
  return (
    <div className="min-h-screen" style={{ background: "linear-gradient(180deg, #071E3D 0%, #123C69 40%, #2563EB 100%)", paddingTop: "env(safe-area-inset-top)" }}>
      {/* Header Section */}
      <div className="px-5 pt-4 pb-6 flex flex-col items-center">
        ...
        <div className="w-16 h-16 bg-white rounded-2xl flex items-center justify-center shadow-lg shadow-black/20">
          <Image src="/logo/logo.svg" alt="SIMPATI" width={36} height={36} />
        </div>

        {/* Brand Text */}
        <h1 className="mt-3 text-white text-xl font-bold tracking-widest">SIMPATI</h1>
        <p className="mt-1 text-white/60 text-xs text-center max-w-[260px] leading-relaxed">
      ...
      <div className="px-5 pt-6 pb-8 text-center">
        <p className="text-xs text-white/50">
          Dengan masuk, Anda menyetujui <span className="text-white/70 font-medium">Syarat & Ketentuan</span>
        </p>
        <span className="inline-block mt-3 px-3 py-1 bg-white/10 rounded-full text-[10px] text-white/50 font-medium">
          v2.0.0
        </span>
      </div>
    </div>
