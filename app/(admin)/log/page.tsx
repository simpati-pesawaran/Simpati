    ...
    <div className="min-h-screen pb-24" style={{ background: "#f1f5f9" }}>
      ...
      {showFilterModal && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center" onClick={() => setShowFilterModal(false)}>
          ...
          <div className="relative bg-white w-full max-w-sm rounded-t-3xl sm:rounded-2xl shadow-xl max-h-[70vh] overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
              <h2 className="text-base font-bold text-gray-900">Filter</h2>
              <button onClick={() => setShowFilterModal(false)} className="w-7 h-7 rounded-full bg-gray-100 flex items-center justify-center"><svg className="w-4 h-4 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></button>
            </div>
            <div className="p-4 space-y-3 overflow-y-auto max-h-[50vh]">
              <div>
                <label className="text-xs font-semibold text-gray-500 mb-1.5 block">Modul</label>
                <div className="grid grid-cols-4 gap-1.5">
                  {[{ value: "all", label: "Semua" }, { value: "agenda", label: "Agenda" }, { value: "gallery", label: "Galeri" }, { value: "usulan", label: "Usulan" }, { value: "user", label: "User" }, { value: "auth", label: "Auth" }].map((opt) => (
                    <button key={opt.value} onClick={() => setFilter({ ...filter, entity: opt.value })} className={"py-1.5 px-2 rounded-lg text-xs font-medium transition-all " + (filter.entity === opt.value ? "bg-indigo-500 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200")}>{opt.label}</button>
                  ))}
                </div>
              </div>
              <div>
                <label className="text-xs font-semibold text-gray-500 mb-1.5 block">Aksi</label>
                <div className="grid grid-cols-5 gap-1.5">
                  {[{ value: "all", label: "Semua" }, { value: "create", label: "Buat" }, { value: "update", label: "Ubah" }, { value: "delete", label: "Hapus" }, { value: "submit", label: "Kirim" }, { value: "approve", label: "Setuju" }, { value: "reject", label: "Tolak" }, { value: "login", label: "Masuk" }, { value: "logout", label: "Keluar" }].map((opt) => (
                    <button key={opt.value} onClick={() => setFilter({ ...filter, action: opt.value })} className={"py-1.5 px-2 rounded-lg text-xs font-medium transition-all " + (filter.action === opt.value ? "bg-indigo-500 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200")}>{opt.label}</button>
                  ))}
                </div>
              </div>
              <div>
                ...
                <div className="flex items-center gap-2">
                  <input type="date" value={filter.dateFrom} onChange={(e) => setFilter({ ...filter, dateFrom: e.target.value })} className="flex-1 px-2 py-2 bg-gray-50 border border-gray-200 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500/40" />
                  <span className="text-gray-400 text-xs">-</span>
                  <input type="date" value={filter.dateTo} onChange={(e) => setFilter({ ...filter, dateTo: e.target.value })} className="flex-1 px-2 py-2 bg-gray-50 border border-gray-200 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500/40" />
                </div>
              </div>
            </div>
            <div className="px-4 py-3 border-t border-gray-100 flex gap-2">
              <button onClick={resetFilters} className="flex-1 py-2.5 bg-gray-100 text-gray-600 text-sm font-medium rounded-xl hover:bg-gray-200 transition-all">Reset</button>
              <button onClick={() => applyFilters(filter)} className="flex-1 py-2.5 bg-gradient-to-r from-indigo-500 to-purple-500 text-white text-sm font-medium rounded-xl hover:opacity-90 transition-all">Terapkan</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showDetailModal && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center" onClick={() => setShowDetailModal(null)}>
          ...
          <div className="relative bg-white w-full max-w-sm rounded-t-3xl sm:rounded-2xl shadow-xl max-h-[75vh] overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
              <h2 className="text-base font-bold text-gray-900">Detail</h2>
              <button onClick={() => setShowDetailModal(null)} className="w-7 h-7 rounded-full bg-gray-100 flex items-center justify-center"><svg className="w-4 h-4 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></button>
            </div>
            <div className="p-4 space-y-3 overflow-y-auto max-h-[55vh]">
              <div className="bg-gray-50 rounded-xl p-3 space-y-2">
                <div className="flex justify-between text-xs"><span className="text-gray-500">Aksi</span><span className="font-medium text-gray-900">{ACTION_CONFIG[showDetailModal.action]?.label || showDetailModal.action}</span></div>
                <div className="flex justify-between text-xs"><span className="text-gray-500">Modul</span><span className="font-medium text-gray-900">{ENTITY_CONFIG[showDetailModal.entity_type]?.label || showDetailModal.entity_type}</span></div>
                <div className="flex justify-between text-xs"><span className="text-gray-500">Pengguna</span><span className="font-medium text-gray-900">{showDetailModal.user_name}</span></div>
                <div className="flex justify-between text-xs"><span className="text-gray-500">Waktu</span><span className="font-medium text-gray-900">{new Date(showDetailModal.created_at).toLocaleString("id-ID")}</span></div>
              </div>
              <div className="bg-indigo-50 rounded-xl p-3">
                <p className="text-xs font-medium text-indigo-800">{showDetailModal.description}</p>
              </div>
              {showDetailModal.old_data && (
                <div>
                  <h4 className="text-xs font-semibold text-gray-600 mb-1.5 flex items-center gap-1"><svg className="w-3.5 h-3.5 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 12H4" /></svg>Sebelum</h4>
                  <pre className="bg-red-50 border border-red-100 rounded-lg p-2 text-xs text-gray-700 overflow-x-auto max-h-32">{JSON.stringify(showDetailModal.old_data, null, 2)}</pre>
                </div>
              )}
              {showDetailModal.new_data && (
                <div>
                  <h4 className="text-xs font-semibold text-gray-600 mb-1.5 flex items-center gap-1"><svg className="w-3.5 h-3.5 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6" /></svg>Sesudah</h4>
                  <pre className="bg-emerald-50 border border-emerald-100 rounded-lg p-2 text-xs text-gray-700 overflow-x-auto max-h-32">{JSON.stringify(showDetailModal.new_data, null, 2)}</pre>
                </div>
              )}
            </div>
          </div>
