...
export default function DashboardPage() {
  ...
  useEffect(() => {
    fetch("/api/dashboard/stats").then(r => r.json()).then(d => d.success && setStats(d.stats)).catch(() => {});
    fetch("/api/agenda?limit=5&upcoming=true").then(r => r.json()).then(d => d.success && setUpcomingEvents(d.data || [])).catch(() => {});
  }, []);
      ...
      <div className="bg-gradient-to-br from-[#1e3a5f] via-[#2563eb] to-[#7c3aed] px-4 pb-6 relative overflow-hidden" style={{ paddingTop: `calc(0.75rem + env(safe-area-inset-top))` }}>
        ...
        <div className="grid grid-cols-2 gap-2.5 mt-5">
          <div className="bg-white/15 backdrop-blur-sm rounded-xl p-3">
            ...
            <p className="text-white text-xl font-bold">{stats.total_kegiatan}</p>
          </div>
          <div className="bg-white/15 backdrop-blur-sm rounded-xl p-3">
            <div className="flex items-center gap-2 mb-1.5">
            ...
            <p className="text-white text-xl font-bold">{stats.total_audiensi}</p>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-4">
          ...
          <div className="grid grid-cols-3 gap-2 mb-2">
            {menuItems.slice(0, 3).map((menu) => (
              <Link key={menu.name} href={menu.href} className="flex flex-col items-center p-2.5 rounded-xl hover:bg-slate-50 transition-all active:scale-95">
                <div className={`w-[52px] h-[52px] rounded-xl bg-gradient-to-br ${menu.color} flex items-center justify-center mb-1.5 shadow-lg`}>
